...
{
	public class ProductSubCategoryManager : BaseManager
	{
		private static readonly string _selectQuery = @"SELECT TOP ({0}) 
			  ...
			  ,[name]
			  ,[category]
		  FROM [ProductSubCategoryGate]
		 Where Status = 0 And Source = 0";

		protected List<ProductSize> _objects;

		public ProductSubCategoryManager()
		{
			_tableName = "ProductSubCategoryGate";
			...
			lock (_lock)
			{
				using (var provider = new DBConnectionProvider())
				{
					using (var reader = provider.Execute(_selectQuery, packSize))
					{
						...
						{
							pack.Add(new ProductSubCategory()
							{
		...
		{
			return JsonConvert.SerializeObject(pack.Select(p => (ProductSubCategory)p).ToList());
		}
	}
}
