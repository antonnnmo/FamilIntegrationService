...
namespace FamilIntegrationService.Providers
{
	public class CRMIntegrationProvider
	{
		...
		public string MakeRequest(string bpmServiceUri, string body)
		{
			HttpWebRequest req;
			...
			catch (WebException e)
			{
				...
				var exceptionStackTrace = e.StackTrace;
				var res = e.Response;
				...
				if (e.Status != WebExceptionStatus.Timeout)
				{
					if (res == null)
					{
						//Logger.LogError("MethodName: {0}, Time: {1}, Message: {2}, StackTrace: {3}. Response is null", "MakeRequest",
						//			dateTimeNow.ToString(), exceptionMessage, exceptionStackTrace);
					}
					else
					{
						using (var responseStream = res.GetResponseStream())
						{
							using (var streamReader = new StreamReader(responseStream))
							{
								...
								//Logger.LogError("MethodName: {0}, Time: {1}, Message: {2}, StackTrace: {3}, Response: {4}", "MakeRequest",
								//									dateTimeNow.ToString(), exceptionMessage, exceptionStackTrace, responseBody);
							}
						}
					}
					...
					{
						var csrf = Authorize(out req, out bpmCookieContainer);
						GlobalCacheReader.SetValue(GlobalCacheReader.CacheKeys.BPMCookie, bpmCookieContainer);
						GlobalCacheReader.SetTemporaryValue(GlobalCacheReader.CacheKeys.BPMCSRF, csrf, TimeSpan.FromMinutes(20));
					}
		...
		private string Request(string bpmServiceUri, string body, out HttpWebRequest req, CookieContainer bpmCookieContainer)
		{
